...
class Agents(object):
    def __init__(self, args):
        ...
        if args.alg.find('random') != -1:
            from policy.random_choose import RandomChoose
            self.policy = RandomChoose(args.n_actions, args)
        else:
    ...
    def choose_random_actions(self, avail_actions):
        return np.random.choice(avail_actions, self.n_agents).tolist()
