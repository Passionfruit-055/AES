...
from agents.Experience_replay import ReplayBuffer
...
class VDN(object):  # DQN类进行强化学习
    ...
    def sample_memory(self):
        ...
        states_ = T.tensor(next_state, dtype=T.float).to(self.q_eval.device)
    ...
    def learn(self):
        ...
        for state, action, reward, next_state in zip(states, actions, rewards, states_):
            q_targets = []
            for s, a, r, n_s in zip(state, action, reward, next_state):
                q_next = self.q_next.forward(n_s)
                max_q_value = T.max(q_next, dim=-1)[0].detach()
                q_target = r.view(max_q_value.size()) + self.gamma * max_q_value
                q_targets.append(q_target)
            q_targets = T.sum(T.tensor(q_targets), dim=0).to(self.q_eval.device)
            for s, a, r, n_s in zip(state, action, reward, next_state):
                q_eval = self.q_eval.forward(s)

                q_eval_replaced = q_eval.clone()
                q_eval_replaced[a] = q_targets
